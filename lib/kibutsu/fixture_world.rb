...
class Kibutsu::FixtureWorld
  ...
  def load(database_connection_url, fixtures_path)
    @database_connection = Kibutsu::DatabaseConnection.new(database_connection_url)
  ...
  def load_fixtures(fixtures_path)
    ...
    Kibutsu::FixtureFinder.new(fixtures_path).fixture_file_paths.each do |fixture_file_path|
      @fixture_tables += Kibutsu::FixtureLoader.new(fixture_file_path, database_connection).fixture_tables
    end
  end
